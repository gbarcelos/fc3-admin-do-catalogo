...
public class GCStorageServiceTest {
  ...
  public void givenPrefix_whenCallsList_shouldRetrieveAll() {
    final var video = Fixture.Videos.resource(VIDEO);
    final var banner = Fixture.Videos.resource(BANNER);
  ...
  public void givenResource_whenCallsDeleteAll_shouldEmptyStorage() {
    ...
    final var actualIds =
        ((List<BlobId>) capturer.getValue()).stream().map(BlobId::getName).toList();

    Assertions.assertTrue(
        expectedIds.size() == actualIds.size() && actualIds.containsAll(expectedIds));
  }
